...
IonIcon.loadFont().then();
MaterialIcon.loadFont().then();
...
export function TakeScanView({navigation}: Props): React.ReactElement {
  const camera = useRef<Camera>(null);
  const cam = Camera.getAvailableCameraDevices().then(devi => {
    console.log('devices', devi),
      devi.forEach(device => {
  ...
  const frameProcessor = useFrameProcessor(frame => {
    ...
    const values = 'disabled example plugin';
